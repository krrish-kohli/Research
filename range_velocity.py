...
class VelocityTrackerPlot:
    def __init__(self, frate):
        ...
        self._velocities = []
        self._times = []  # store times instead of frames

    def update(self, frame_number, velocity):
        # Compute time from frame_number based on frame rate
        current_time = frame_number * (1.0 / self._frate)

        self._velocities.append(velocity)
        self._times.append(current_time)

        self._velocity_line.set_xdata(self._times)
    ...
    def get_data(self):
        """Return the collected times (s) and velocities (m/s)."""
...
if __name__ == '__main__':
    ...
    with DeviceFmcw() as device:
        ...
        vel_plot = VelocityTrackerPlot(frate)
        ...
        # Retrieve the recorded data (times and velocities)
        times, velocities = vel_plot.get_data()

        # Create a DataFrame from the data: now it's Time (s) vs Velocity (m/s)
        df = pd.DataFrame({"Time (s)": times, "Velocity (m/s)": velocities})
